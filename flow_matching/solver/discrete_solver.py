        ...
        **model_extras,
        ...
        with ctx:
            for i in range(n_steps):
                ...
                if temperature != 1.0:
                    log_p = torch.log(p_1t.clamp(min=1e-8))
                    p_1t = F.softmax(log_p / temperature, dim=-1)
    ...
    def sample(
        ...
        **model_extras,
        ...
        Args:
            ...
            return_intermediates (bool): If True then return intermediate time steps according to time_grid. Defaults to False.
            verbose (bool): Whether to print progress bars. Defaults to False.
            temperature (float): Softens/sharpens the CTMC posterior before sampling; >1 increases diversity. Defaults to 1.0 (no change).
            **model_extras: Additional input for the model.
        ...
        with ctx:
            ...
            for i in range(n_steps):
                ...
                if temperature != 1.0:
                    if p_1t_img is not None:
                        log_p = torch.log(p_1t_img.clamp(min=1e-8))
                        p_1t_img = F.softmax(log_p / temperature, dim=-1)
                    if p_1t_txt is not None:
                        log_p = torch.log(p_1t_txt.clamp(min=1e-8))
                        p_1t_txt = F.softmax(log_p / temperature, dim=-1)
                if p_1t_txt is None:
